...
TCB* spawn_thread(PCB* pcb, void (*func)())
{
	...
	//edits
	tcb->priority=MAX_PRIORITY/2;	// initialise priority
	tcb->counter_aging=0;	//initialise counter for aging
	tcb->ptcb= NULL;
	//end of aging
...
static TCB* sched_queue_select(TCB* current)
{
	...
	TCB* next_thread = sel==NULL ?NULL: sel->tcb; /* When the list is empty, this is NULL */
...
// edits
void priority_inversion(TCB* tcb){
	if( tcb -> curr_cause == SCHED_MUTEX && tcb->last_cause ==SCHED_MUTEX){
		if(tcb->priority >0 && tcb->priority < MAX_PRIORITY - 1){
			tcb->priority++;	// if mutex increase priotiy to wait
		}
	}
}
// IO go to high priority , Quantum go to lower priority
void switch_priority(enum SCHED_CAUSE cause , TCB* curr){
	if(cause == SCHED_IO && curr->priority >0 ) curr-> priority=0;
	else if(cause == SCHED_QUANTUM && curr->priority < MAX_PRIORITY-1)curr->priority++;
}
	...
	for(int i =1 ; i < MAX_PRIORITY ; i++){
		if(!is_rlist_empty(&SCHED[i])){
			for(int y =0;y < rlist_len(&SCHED[i]); y++){
				curr=rlist_pop_front(&SCHED[i]);
				assert(curr!=NULL);
				curr->tcb->counter_aging++;
				if(curr->tcb->counter_aging == MAX_AGING){
					curr->tcb->priority--;
					curr->tcb->counter_aging=0;
				}
				rlist_push_back(&SCHED[curr->tcb->priority],curr);
			}
		}
	}
	Mutex_Unlock(&sched_spinlock);
