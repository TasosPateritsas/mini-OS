...
void sys_Exit(int exitval)
{
  ...
  kernel_broadcast(&ptcb->cond_var);
 // end of edits 
...
Fid_t sys_OpenInfo()
{
  ...
  ICB* icb =(ICB*)xmalloc(sizeof(ICB));
  icb->write_pos=0;
  icb->reader_pos=0;
  fcb->streamobj=icb;
  fcb->streamfunc = & sysinfo_func;
  ...
  for(int i =0; i<MAX_PROC;i++){
      ...
      if(pcb->pstate!=FREE){
        ...
        info->alive=(pcb->pstate==ALIVE)?1:0;
        info->thread_count =pcb->counter_ptcb+1;
        info->main_task=pcb->main_task;
        info->argl=pcb->argl;
        int count =(pcb->argl<PROCINFO_MAX_ARGS_SIZE)? pcb->argl : PROCINFO_MAX_ARGS_SIZE;
        for (int  j = 0; j < count; j++) {
          info->args[j]=((char *) pcb->args)[j];
        }
        memcpy(&icb->buffer[icb->write_pos],info,sizeof(procinfo));
        icb->write_pos=icb->write_pos +sizeof(procinfo);
...
int info_read(void* infocb,char *buffer,unsigned int size ){
  ICB* icb = (ICB*) infocb;
  int i = 0;
  for ( i = 0; i < size; i++) {
    if (icb->reader_pos >= icb->write_pos) {
      return i;
    }
    buffer[i]=icb->buffer[icb->reader_pos];
    icb->reader_pos++;
  }
  return i;
}
